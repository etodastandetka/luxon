#!/bin/bash
# Скрипт для обновления всех проектов на сервере
# Использование: ./UPDATE_ALL.sh

set -e  # Остановка при ошибке

echo "🔄 Обновление всех проектов..."
echo ""

# Цвета для вывода
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
NC='\033[0m' # No Color

# Функция для обновления проекта
update_project() {
    local dir=$1
    local name=$2
    
    echo -e "${YELLOW}📦 Обновление $name...${NC}"
    echo "   Путь: $dir"
    
    if [ ! -d "$dir" ]; then
        echo -e "${RED}   ❌ Директория не найдена: $dir${NC}"
        return 1
    fi
    
    cd "$dir" || return 1
    
    # Проверяем, это git репозиторий?
    if [ ! -d ".git" ]; then
        echo -e "${YELLOW}   ⚠️  Это не git репозиторий, пропускаем${NC}"
        return 0
    fi
    
    # Сохраняем локальные изменения
    if [ -n "$(git status --porcelain)" ]; then
        echo "   💾 Сохраняем локальные изменения в stash..."
        git stash push -m "Auto-stash before update $(date +%Y-%m-%d_%H-%M-%S)" || true
    fi
    
    # Обновляем код
    echo "   ⬇️  Выполняем git pull..."
    if git pull; then
        echo -e "${GREEN}   ✅ $name обновлен успешно${NC}"
        
        # Пытаемся применить stash обратно
        if git stash list | grep -q "Auto-stash"; then
            echo "   📦 Пытаемся применить сохраненные изменения..."
            git stash pop || echo "   ⚠️  Конфликты при применении stash (можно игнорировать)"
        fi
        
        # Если это Next.js проект, собираем его
        if [ -f "package.json" ] && grep -q "next" package.json; then
            echo "   🔨 Собираем Next.js проект..."
            if npm run build; then
                echo -e "${GREEN}   ✅ Сборка успешна${NC}"
            else
                echo -e "${RED}   ❌ Ошибка при сборке${NC}"
            fi
        fi
        
        return 0
    else
        echo -e "${RED}   ❌ Ошибка при git pull${NC}"
        return 1
    fi
}

# Обновляем все проекты
# ПРАВИЛЬНЫЕ ПУТИ согласно ecosystem.config.js:
# - Админка: /var/www/luxon/admin_nextjs
# - Мини-приложение: /var/www/luxon/app (НЕ app/app!)
# - Бот: /var/www/luxon/bot

echo "1️⃣  Обновление админки..."
update_project "/var/www/luxon/admin_nextjs" "Админка" || true

echo ""
echo "2️⃣  Обновление мини-приложения..."
update_project "/var/www/luxon/app" "Мини-приложение" || true

echo ""
echo "3️⃣  Обновление корневого репозитория (если есть)..."
update_project "/var/www/luxon" "Корневой репозиторий" || true

echo ""
echo "🔄 Перезапуск PM2 процессов..."
pm2 restart all || true

echo ""
echo -e "${GREEN}✅ Обновление завершено!${NC}"
echo ""
echo "📊 Статус PM2:"
pm2 status

