#!/bin/bash
# Скрипт для безопасного обновления на сервере
# Сохраняет локальные изменения в stash, обновляет код, затем пытается применить stash

cd /var/www/ls/admin_nextjs || exit 1

echo "📦 Сохраняем локальные изменения в stash..."
git stash push -m "Local changes before update $(date +%Y-%m-%d_%H-%M-%S)"

echo "⬇️  Обновляем код из репозитория..."
git pull

if [ $? -eq 0 ]; then
    echo "✅ Код успешно обновлен"
    
    echo "📦 Пытаемся применить сохраненные изменения..."
    git stash pop
    
    if [ $? -ne 0 ]; then
        echo "⚠️  Есть конфликты при применении stash. Проверьте изменения:"
        echo "   git stash list - для просмотра сохраненных изменений"
        echo "   git diff - для просмотра текущих конфликтов"
    else
        echo "✅ Локальные изменения успешно применены"
    fi
else
    echo "❌ Ошибка при обновлении кода"
    
fi

echo "🔨 Устанавливаем зависимости (если нужно)..."
npm install --production=false

echo "🔨 Генерируем Prisma Client..."
npm run db:generate

echo "🔨 Собираем проект..."
npm run build

if [ $? -eq 0 ]; then
    echo "✅ Сборка успешна"
    
    echo "🔄 Перезапускаем PM2..."
    pm2 restart luxon-admin
    
    echo "✅ Обновление завершено!"
    echo ""
    echo "📊 Статус PM2:"
    pm2 status
else
    echo "❌ Ошибка при сборке"
    echo "Проверьте логи выше для деталей"
    exit 1
fi

