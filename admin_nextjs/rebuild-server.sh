...
# Принудительно обновляем до последней версии из репозитория
# Это гарантирует, что все конфигурационные файлы актуальны
echo "   Получаю последние изменения из удаленного репозитория..."
git fetch origin main

# Проверяем, есть ли локальные изменения
if ! git diff --quiet HEAD origin/main 2>/dev/null; then
    echo "⚠️  Обнаружены различия с удаленной версией, обновляю файлы..."
    git reset --hard origin/main
    echo "✅ Файлы обновлены до последней версии"
else
