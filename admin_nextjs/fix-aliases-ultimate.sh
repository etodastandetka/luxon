#!/bin/bash
# УЛЬТИМАТИВНЫЙ скрипт - исправляет ВСЁ правильно

set -e

echo "🔧 УЛЬТИМАТИВНОЕ исправление всех алиасов..."
echo ""
...
# 1. Исправляем app/api/auth/login/route.ts - там точно остался 'a/lib/auth'
...
# 2. Проверяем и исправляем app/api/auth/2fa/* - там нужен путь ../../../../../lib/
echo "📝 Проверяю app/api/auth/2fa/*..."
for file in app/api/auth/2fa/*/route.ts; do
    if [ -f "$file" ]; then
        echo "  ✅ Проверяю $file"
        # Заменяем все варианты на правильный путь (6 уровней вверх)
...
# 3. Исправляем ВСЕ файлы в app/api (4 уровня вверх для большинства)
echo "📝 Исправляю все файлы в app/api/..."
...
check_file() {
    local file="$1"
    if [ -f "$file" ]; then
        count=$(grep -c "from ['\"]a/\|from ['\"]@/" "$file" 2>/dev/null || echo "0")
        if [ "$count" -gt 0 ]; then
            echo "  ⚠️  $file: найдено алиасов: $count"
            grep "from ['\"]a/\|from ['\"]@/" "$file" | head -3
        else
