#!/bin/bash
# Безопасная версия OAuth2 flow - не прерывает SSH сессию
# Использование: bash scripts/curl-oauth-safe.sh

echo "🧪 Полный OAuth2 flow через curl (безопасная версия)"
echo "============================================================"
echo ""

# Очищаем старые cookies
rm -f /tmp/mobcash_cookies.txt
...
LOCATION1=$(echo "$RESPONSE1" | grep -i "^location:" | head -1 | cut -d' ' -f2- | tr -d '\r')
LOGIN_CHALLENGE=$(echo "$LOCATION1" | grep -oP 'login_challenge=\K[^&]*' || echo "")
...
# Проверяем статус
STATUS2=$(echo "$RESPONSE2" | head -1 | grep -oP 'HTTP/\d+ \K\d+' || echo "")

if [ "$STATUS2" != "302" ] && [ "$STATUS2" != "301" ]; then
  echo "❌ Ошибка: неожиданный статус $STATUS2"
  echo "Ответ (первые 20 строк):"
...
if [ -z "$LOGIN_VERIFIER" ]; then
  echo "❌ Ошибка: не удалось получить login_verifier"
...
RESPONSE3=$(curl -s --max-time 30 -i -L "$LOCATION2" \
  --header 'Accept: application/json, text/plain, */*' \
  --cookie-jar /tmp/mobcash_cookies.txt \
  --cookie /tmp/mobcash_cookies.txt 2>&1)

LOCATION3=$(echo "$RESPONSE3" | grep -i "^location:" | head -1 | cut -d' ' -f2- | tr -d '\r')
CONSENT_CHALLENGE=$(echo "$LOCATION3" | grep -oP 'consent_challenge=\K[^&]*' || echo "")
...
# Шаг 1.4: Получение Access Token
echo "🔐 Шаг 1.4: Получение Access Token..."
echo ""

RESPONSE4=$(curl -s --max-time 30 -i -X POST "https://admin.mob-cash.com/authentication/consent?consent_challenge=${CONSENT_CHALLENGE}" \
  --header 'Accept: application/json, text/plain, */*' \
  --header 'Accept-Language: en,ru;q=0.9,ru-RU;q=0.8,en-US;q=0.7' \
  --header 'Connection: keep-alive' \
  --header 'Origin: https://app.mob-cash.com' \
...
LOCATION4=$(echo "$RESPONSE4" | grep -i "^location:" | head -1 | cut -d' ' -f2- | tr -d '\r')
AUTH_CODE=$(echo "$LOCATION4" | grep -oP 'code=\K[^&]*' || echo "")

if [ -z "$AUTH_CODE" ]; then
  echo "❌ Ошибка: не удалось получить authorization code"
  echo "Location: $LOCATION4"
  echo ""
  echo "⚠️  Продолжаем работу..."
  return 0 2>/dev/null || true
fi

echo "✅ Authorization Code получен: ${AUTH_CODE:0:50}..."
echo ""

# Обмениваем код на токен
echo "🔐 Шаг 1.5: Обмен authorization code на access token..."
echo ""

TOKEN_RESPONSE=$(curl -s --max-time 30 -X POST 'https://admin.mob-cash.com/hydra/oauth2/token' \
  --header 'Accept: application/json' \
  --header 'Content-Type: application/x-www-form-urlencoded' \
  --data-urlencode "grant_type=authorization_code" \
  --data-urlencode "code=${AUTH_CODE}" \
  --data-urlencode "client_id=4e779103-d67b-42ef-bc9d-ab5ecdec40f8" \
  --data-urlencode "redirect_uri=https://app.mob-cash.com" 2>&1)
...
# Шаг 1.6: Получение User ID
echo "🔐 Шаг 1.6: Получение User ID..."
echo ""
...
USER_PROFILE_RESPONSE=$(curl -s --max-time 30 -X POST 'https://admin.mob-cash.com/api/' \
  ...
  --data '[{"jsonrpc":"2.0","id":11,"method":"user.profile","params":{}}]' 2>&1)
...
# Шаг 1.7: Логин на кассу (Session ID)
echo "🔐 Шаг 1.7: Логин на кассу (получение Session ID)..."
echo ""
...
LOGIN_RESPONSE=$(curl -s --max-time 30 -X POST 'https://admin.mob-cash.com/api/' \
  ...
  --header 'User-Agent: Mozilla/5.0 (iPhone; CPU iPhone OS 18_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.5 Mobile/15E148 Safari/604.1' \
  --header 'x-request-source: pwa' \
  --data "[{\"jsonrpc\":\"2.0\",\"id\":12,\"method\":\"mobile.login\",\"params\":{\"location\":{\"lat\":42.845778,\"lon\":74.568778},\"cashboxCode\":1001098,\"userID\":\"${USER_ID}\"}}]" 2>&1)
...
if [ -n "$ACCESS_TOKEN" ] && [ -n "$USER_ID" ]; then
  ...
  echo "⚠️  АВТОРИЗАЦИЯ НЕ ЗАВЕРШЕНА"
  echo "============================================================"
  echo "Некоторые шаги не выполнились, но скрипт завершился безопасно"
fi
echo ""
echo "============================================================"
