#!/bin/bash
# Полный OAuth2 flow - автоматически извлекает LoginChallenge и использует его
# Использование: bash scripts/curl-oauth-complete.sh
...
if [ -z "$LOGIN_CHALLENGE" ]; then
  ...
  echo ""
  echo "⚠️  Скрипт завершен с ошибкой, но сессия SSH не прервана"
...
echo "Используем LoginChallenge: $LOGIN_CHALLENGE"
echo ""
