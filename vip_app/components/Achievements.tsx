      ...
      <div className="grid grid-cols-2 gap-2">
        {achievements.length > 0 ? (
          achievements.map((achievement) => (
            ...
            {achievement.unlocked && (
              <div className="text-xs text-sky-300 mt-1">✓ Получено</div>
            )}
          </div>
          ))
        ) : null}
      ...
      {selectedAchievement && (
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => setSelectedAchievement(null)}
              className="w-full mt-4 py-3 bg-sky-500 text-white rounded-lg font-semibold hover:bg-sky-600 transition-colors"
            >
              Понятно
            </button>
          </div>
