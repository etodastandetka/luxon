...
На сервере старая версия. Выполните:
...
## Вариант 2: Переклонировать заново (РЕКОМЕНДУЕТСЯ)

```bash
# Сохраните важные файлы (если есть .env файлы)
cd /var/www
mv luxon luxon_backup

# Клонируйте свежую версию
git clone https://github.com/etodastandetka/ls.git luxon

# Скопируйте .env файлы из бэкапа (если были)
cp luxon_backup/app/.env.local luxon/app/.env.local 2>/dev/null || true
cp luxon_backup/admin_nextjs/.env luxon/admin_nextjs/.env 2>/dev/null || true
cp luxon_backup/bot/.env luxon/bot/.env 2>/dev/null || true

# Удалите старый бэкап (когда убедитесь что все работает)
# rm -rf luxon_backup
```

## После обновления выполните скрипт:
